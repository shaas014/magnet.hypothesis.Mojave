...
#rates needed
frequency <- frequency %>% mutate(rate.per.flower = (count/mean.floral.density)) %>% mutate(rate.per.flower.hr = rate.per.flower/net.time)
...
#visitations
m <- glm(rate.per.flower~net.treatment + insect.RTU %in% rep + mean.temp, family = "poisson", data = freq.2015)
anova(m, test = "Chisq")
require(lsmeans)
