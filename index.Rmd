...
frequency <- data %>% group_by(year, day, net.treatment, rep, microsite, annuals, target.flowers) %>% summarise(net.time = sum(total.duration), mean.visitation.duration = mean(weighted.visitation.duration), mean.floral.density = mean(floral.density), count = n()) 
...
ggplot(frequency, aes(microsite, rate)) + geom_boxplot() + facet_wrap(~year*target.flowers)
...
ggplot(frequency, aes(microsite, mean.visitation.duration)) + geom_boxplot() + facet_wrap(~year*annuals*target.flowers)
...
#target flowers
ggplot(freq.2015, aes(rate, fill = target.flowers)) + geom_histogram() + facet_wrap(~microsite)
#ha, only larrea in 2015

ggplot(freq.2016, aes(rate, fill = target.flowers)) + geom_histogram() + facet_wrap(~microsite)
