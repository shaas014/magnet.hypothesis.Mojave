...
freq.rtu <- frequency %>% filter(insect.RTU != "none") %>% filter(rate.per.flower.hr <0.4) %>% filter(net.visitation <2)
...
#denominator is mean.time of recordings across all reps
...
#Simple models####
#visitations
m1 <- glm(rate.per.flower~net.treatment + insect.RTU %in% rep + mean.temp, family = "poisson", data = freq.2015)
anova(m1, test = "Chisq")
require(lsmeans)
lsmeans(m1, pairwise~net.treatment, adjust="tukey")

m2 <- glm(rate.per.flower~net.treatment*insect.RTU %in% rep + mean.temp, family = "poisson", data = freq.2015)
anova(m2, test = "Chisq")
...
m <- glm(rate.per.flower~net.treatment + insect.RTU %in% rep + mean.temp, family = "poisson", data = freq.2016)
anova(m, test = "Chisq")
lsmeans(m, pairwise~net.treatment, adjust="tukey")

m <- glm(rate.per.flower.hr~net.treatment + insect.RTU %in% rep + mean.temp, family = "poisson", data = freq.2015)
anova(m, test = "Chisq")
lsmeans(m, pairwise~net.treatment, adjust="tukey")

m <- glm(rate.per.flower.hr~net.treatment + insect.RTU %in% rep + mean.temp, family = "poisson", data = freq.2016)
anova(m, test = "Chisq")
lsmeans(m, pairwise~net.treatment, adjust="tukey")

#duration of visits
m <- glm(net.visitation~net.treatment + insect.RTU %in% rep + mean.temp, family = "poisson", data = freq.2015)
anova(m, test = "Chisq")
lsmeans(m, pairwise~net.treatment, adjust="tukey")

m <- glm(net.visitation~net.treatment + insect.RTU %in% rep, family = "poisson", data = freq.2016)
anova(m, test = "Chisq")
lsmeans(m, pairwise~net.treatment, adjust="tukey")
lsmeans(m, pairwise~insect.RTU %in% rep, adjust="tukey")
