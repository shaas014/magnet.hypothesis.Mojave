...
expt2 <- expt2 %>% mutate(treatment = ifelse(net.treatment %in% c("Larrea flowers with annuals"), "Larrea flowers + annuals", ifelse(net.treatment %in% c("Larrea flowers without annuals"), "Larrea flowers - annuals", "NA")))
...
#temperature####
ggplot(expt1, aes(mean.temp, rate.per.flower, color = insect.RTU)) + geom_point() + ylab("rate per flower") + geom_smooth(method = "lm") + facet_wrap(~year) + scale_color_brewer(palette = "Blues")

#floral density####
ggplot(expt1, aes(net.floral.density, count, color = insect.RTU)) + geom_point() + ylab("count") + geom_smooth(method = "lm") + facet_wrap(~year) + scale_color_brewer(palette = "Blues")

#expt2####
#visitations####
expt2 <- expt2 %>% filter(rate.per.flower < 0.25)
ggplot(expt2, aes(treatment, rate.per.flower, fill = insect.RTU)) + geom_boxplot() + ylab("rate per flower") + scale_fill_brewer(palette = "Blues") + facet_grid(~year)

#visit durations####
#net visitation time
expt2 <- expt2 %>% filter(net.visitation < 0.02)
ggplot(expt2, aes(treatment, net.visitation, fill = insect.RTU)) + geom_boxplot() + ylab("net duration of visits per hour") + scale_fill_brewer(palette = "Blues") + facet_grid(~year)

#temperature####
ggplot(expt2, aes(mean.temp, rate.per.flower, color = insect.RTU)) + geom_point() + ylab("rate per flower") + geom_smooth(method = "lm") + facet_wrap(~year) + scale_color_brewer(palette = "Blues")
...
###Interpretation  
1. Taxa specificity in plant-pollinator associations is critical for both foundation and annual plant species 
2. Larrea flowers are less attractive to bees relative to annuals within this ecosystem.  
