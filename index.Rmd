...
frequency <- data %>% group_by(year, day, net.treatment, rep, microsite, annuals, flowering.shrub) %>% summarise(net.time = sum(visitation.duration), mean.visitation.duration = mean(weighted.visitation.duration), mean.floral.density = mean(floral.density), count = n()) 
...
#higher-order treatment patterns
ggplot(frequency, aes(net.treatment, rate)) + geom_boxplot()

ggplot(frequency, aes(microsite, rate)) + geom_boxplot() + facet_wrap(~annuals)
