...
data <- data %>% mutate(visitation.rate = as.duration(visitation.duration)/as.duration(total.duration))

#frequency counts in a separate dataframe (weighted by duration of recording)
frequency <- data %>% group_by(year, day, net.treatment, rep, microsite, annuals, flowering.shrub) %>% count(visitation.rate)

frequency2 <- data %>% group_by(year, day, net.treatment, rep, microsite, annuals, flowering.shrub) %>% mutate(count = n())

frequency3 <- data %>% group_by(year, day, net.treatment, rep, microsite, annuals, flowering.shrub) %>% summarise(net.time = sum(visitation.duration))

  
#mutate(net.total.duration = sum(total.duration))mean.floral.density = mean(floral.density, total.flowers.visited = sum(flowers.visited), mean.visitation.rate = mean(visitation.rate)))total visitation.duration per experimental day
