...
2. The best fit model included interaction terms (not additive) and nests insect.RTU within reps. However, contrasts of AIC scores suggest additive model best fit and given data viz and non-orthogonality issues, a compelling argument can be made for additive.
3. It is acceptable to treat each year separately and not in a single model.
...
5. There is no support for the double-magnet prediction.  

###Supporting analyses
```{r supporting, warning=FALSE, message=FALSE}
#test multiple vector data structure####

#test variation in temp
